...
public class WF_CCBill extends WebContralBase
{
	...
	*/
	...
	/** 
	 方法编号
	*/
	public final String getMethodNo() throws Exception {
	...
	*/
	...
	public final String MyDict_DoFlowBatchBaseData_StartFlow() throws Exception {
		...
		qo.DoQuery();
		GEDtl gedtl = null;
		String mapdtlNo = "ND" + Integer.parseInt(this.getFK_Flow()) + "01" + "Dtl1";
		GEDtls gedtls = new GEDtls(mapdtlNo);
		gedtls.Retrieve(GEDtlAttr.RefPK, workid, null);
		for (GEEntity en : ens.ToJavaList())
		{
		...
		return "../MyFlow.htm?FK_Flow=" + this.getFK_Flow() + "&WorkID=" + workid;
	}
	/** 
	 执行流程:变更基础资料
	...
	*/
	public final String MyDict_DoFlowEtc_StartFlow() throws Exception {
		...
		for (Attr item : attrs.ToJavaList())
		{
			if (Glo.getFlowFields().contains("," + item.getKey() + ",") == true)
			{
				continue;
			}
	...
	*/
	public final String DoMethod_ExeSQL() throws Exception {
		...
		String workID = String.valueOf(this.getWorkID());
	...
	*/
	...
	public final String Search_ToolBar() throws Exception {
		...
		if (md.getDTSearchWay ()!= DTSearchWay.None && md.GetParaInt("DTShowWay", 0) == 1)
		{
			...
			try
			{
				String sql = "SELECT min(" + md.getDTSearchKey ()+ ") From " + en.getEnMap().getPhysicsTable();
		...
		dt.TableName = "Attrs";
		//dt.setPrimaryKey = new DataColumn[] {dt.Columns.get("Field")};
		ds.Tables.add(dt);
		...
		for (String ctrl : ctrls)
		{
			...
			if (ds.contains(attr.getKey()) == false)
			{
				...
				if (mapExt != null)
				{
					...
					fullSQL = Glo.DealExp(fullSQL, null, null);
		...
		{
			methods.getGetNewEntity().CheckPhysicsTable();
	...
	*/
	public final String Search_MapAttrForDB() throws Exception {
		...
		if (dblist.getDBType() == 0)
		{
			...
			explist = Glo.DealExp(explist, null);
	...
	public final String SearchDB_UrlSearchData(String urlExt, String postData)
	{
		if (urlExt.contains("http") == false)
		{
			...
			if (SystemConfig.getIsBSsystem())
			{
				...
				{
					urlExt = urlExt.replace("@AppPath", "http://" + host + WebContralBase.getRequest().getRequestURI()); //BP.Sys.Base.Glo.Request.ApplicationPath
				}
				else
				{
					urlExt = "http://" + WebContralBase.getRequest().getRequestURI() + urlExt;
				}
			}
	...
	public final String SearchDB_Init() throws Exception {
		...
		if (md.getDBType() == 0)
		{
			...
			expCount = Glo.DealExp(expCount, null, null);
			expPageSize = Glo.DealExp(expPageSize, null, null);
	...
	*/
	...
	public final DataTable Search_Data() throws Exception {
		...
		if (DataType.IsNullOrEmpty(hidenField) == false)
		{
			...
			for (String field : hidenField.split("[;]", -1))
			{
				...
				if (SystemConfig.getAppCenterDBFieldIsParaDBType() == true)
				{
					String valType = (String) bp.sys.base.Glo.GenerRealType(attrs, str[0], val);
	...
	public final String ImpData_Done() throws Exception {
		...
		if (impWay == 0)
		{
			...
			for (DataRow dr : dt.Rows)
			{
				...
				{
					no = bp.tools.StringHelper.padLeft(no, Integer.parseInt(codeStruct), '0');
				}
	...
	private String SetEntityAttrVal(String no, DataRow dr, Attrs attrs, GEEntity en, DataTable dt, int saveType, FrmBill fbill) throws Exception {
		...
		if (gb.RetrieveFromDBSources() == 0)
		{
			...
			gb.setFKFrmTree(fbill.getFKFormTree()); //单据类别.
		...
		for (Attr item : attrs.ToJavaList())
		{
			...
			else
			{
				if (item.getKey().equals("CI_SmallBusinessFormatCode"))
				{
					...
					mapExt.setMyPK(mypk);
					if (mapExt.RetrieveFromDBSources() == 1 && mapExt.getDoWay() == 3 && DataType.IsNullOrEmpty(mapExt.getTag3()) == false)
					{
						String newVal = "," + val + ",";
						String keyVal = "";
						...
						{
							if (drr.get("Name") != null && newVal.contains("," + drr.get("Name").toString() + ",") == true)
							{
								keyVal += drr.get("No").toString() + ",";
							}
						}
	...
	*/
	...
	public final String DictFlow_MapAttrs() throws Exception {
		DataSet ds = new DataSet();
		String fk_mapData = "ND" + Integer.parseInt(this.getFK_Flow()) + "01";
		...
		qo.AddWhere(MapAttrAttr.FK_MapData, "ND" + Integer.parseInt(this.getFK_Flow()) + "Rpt");
		qo.addAnd();
		...
		UserRegedit ur = new UserRegedit(WebUser.getNo(), "ND" + Integer.parseInt(this.getFK_Flow()) + "Rpt_SearchAttrs");
	...
	public final String DictFlow_Search() throws Exception {
		...
		Cash.getSQL_Cash().remove(fk_mapData);
	...
	public final String DictFlow_Qcode() throws Exception {
		String state = "FlowNo_" + this.getFK_Flow() + "|OrgNo_" + WebUser.getOrgNo() + "|FrmID_" + this.getFrmID() + "|FrmOID_" + this.GetRequestVal("FrmOID");
		//回调url
		String redirect_uri = URLEncoder.encode("http://www.ccbpm.cn/WF/CCBill/DictFlowStart.htm");
		//授权链接
